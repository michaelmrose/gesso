(ns gesso.choreo.semantics
  "Executable global semantics for the current Gesso Choreo core.
   ...
   :await
     ...
     An await event contract may additionally declare an
     :authoritative-observation descriptor naming the logical authority, the
     observation/read operation, and the required semantic key carrying its
     authoritative basis. Such a reread is an explicit role-local semantic
     occurrence. It can establish abstract semantic values without pretending
     that the reread itself is a new authoritative mutation or a participant
     communication.
...
(defn- declared-authoritative-observation
  [state event]
...
(defn- validate-authoritative-observation!
  [state-id event required observation]
  (require-map!
...
(defn- validate-await-event-contract!
  ...
  (let [required
    ...
    (when (contains? contract :authoritative-observation)
      (validate-authoritative-observation!
       state-id
       event
       required
...
(defn- environment-data-valid?
  ...
  (when-some [data
    ...
    (let [event-id
          ...
          authoritative-observation
          (declared-authoritative-observation
           state
           event-id)

          basis-key
          (:basis-key authoritative-observation)]
      ...
      (and
       ...
       (or
        (nil? authoritative-observation)
        (and
         (contains? data basis-key)
         (some? (get data basis-key))))))))
...
(defn enabled?
  "True when event is admitted by the current global state.
   ...
   - an :authoritative-observation event carries a present, non-nil value under
     its declared required :basis-key.
      ...
      (if authoritative-observation
        (let [basis-key
          ...
          {:kind :authoritative-observation
           ...
           :role (:role state)
           :event event-id
           :authority (:authority authoritative-observation)
           :observation (:observation authoritative-observation)
           :basis-key basis-key
           :basis (get semantic-data basis-key)
           :data semantic-data})
...
(defn transition
  "Apply one GlobalStep.
   ...
   it; this function does not itself run model code or transactions. An
   authoritative-observation history occurrence records a role-local reread at
   an explicit basis and is intentionally not added to the distributed
   observable trace. Transport, browser effects, and role-local knowledge remain
   realization/proof concerns."
  [configuration event]
  (let [configuration'
        (require-configuration!
         configuration)]
