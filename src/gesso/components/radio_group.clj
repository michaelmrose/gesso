...
(defn radio
  ...
  [& args]
  (let [[opts _children]
        (normalize-component-args args)

        {:keys [props class attrs]}
        (split-opts opts)
...
(defn radio-group
  ...
  Long form:
    (radio-group {} ...custom radio rows...)

  HTMX notes:
  - HTMX attributes may be passed through :attrs on the fieldset.
  - This is useful when changing the selected option should swap another region,
    reveal a dependent form fragment, or trigger server-side branching UI.
